...
async function exportVideo(){
 ...
 try{
  ...
  check(job.signal);const name=filename(spec.extension),url=URL.createObjectURL(blob),file=new File([blob],name,{type:spec.mime});
  videoResult={blob,url,file};
  ...
  $('download').href=url;$('download').download=name;$('download').textContent='Download '+spec.label;
  $('resultInfo').textContent=`${canvas.width} × ${canvas.height} · ${fps} fps · ${spec.label}${transparent?' · Transparent':''} · ${(blob.size/1048576).toFixed(1)} MB · ${format(state.duration)}`+(canPreview?'':' · Download to view in a compatible video editor.');
  $('result').hidden=false;let canShare=false;try{canShare=!!navigator.canShare?.({files:[file]});}catch{}$('share').hidden=!canShare;
  status('Your '+spec.label+' is ready. Use Download '+spec.label+' or Share / Save to Files.');
...
$('export').onclick=exportVideo;$('cancel').onclick=()=>{state.job?.abort();$('cancel').disabled=true;progress($('progress').value/100,'Canceling…');};$('share').onclick=async()=>{if(!videoResult)return;try{await navigator.share({files:[videoResult.file]});}catch(e){if(e.name!=='AbortError')status('Use the Download button to save this video.',true);}};
...
for(const key of settingsKeys)$(key).onchange=()=>{remember();state.settings[key]=['uppercase','transparent'].includes(key)?$(key).checked:key==='fps'?normalizeFps($(key).value):['motion','groupSize'].includes(key)?Number($(key).value):$(key).value;changed();};
function theme(background,foreground){remember();Object.assign(state.settings,{background,foreground});syncSettings();changed();}$('lightTheme').onclick=()=>theme('#f4f4f1','#111111');$('darkTheme').onclick=()=>theme('#000000','#ffffff');
